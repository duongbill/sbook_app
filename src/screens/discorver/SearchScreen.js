...
const SearchScreen = ({ navigation }) => {
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
            ...
            <Animated.ScrollView
                style={[styles.scrollContent, { transform: [{ translateY: slideAnim }] }]}
                showsVerticalScrollIndicator={false}
            >
                <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Kết quả tìm kiếm</Text>
                {books.length === 0 ? (
                    <Text style={[styles.noResultText, { color: theme.colors.text }]}>Không tìm thấy sách đang tìm kiếm.</Text>
                ) : (
                    <FlatList
                        data={books}
                ...
                {searchQuery.trim() === '' && (
                    <>
                        <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Tác giả được tìm nhiều</Text>
                        <FlatList
                            data={authors}
                            keyExtractor={item => item.id}
                            renderItem={({ item }) => (
